...
type ChainListener = (...args: any[]) => void;

@Injectable()
export class ElectionsSyncService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly chainListeners: Array<{
    eventName: string;
  ...
  onModuleDestroy() {
    ...
    if (this.chainListeners.length === 0) {
      return;
    }

    const contract = this.blockchainService.getContract();
    for (const { eventName, listener } of this.chainListeners) {
  ...
  async syncElectionEvents(contractElectionId: number) {
    ...
    await this.refreshElectionVoteCounts(dbElectionId);
  ...
  private async refreshElectionVoteCounts(electionId: number) {
    const counts = await this.prisma.voteEvent.groupBy({
      by: ["candidateIndex"],
      where: { electionId },
      _count: { _all: true },
    });
    const recordedTotalVotes = counts.reduce(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const item of counts) {
        await tx.candidate.updateMany({
          where: { electionId, index: item.candidateIndex },
          data: { voteCount: item._count._all },
        });
      }
      ...
      await tx.election.update({
        ...
        data: {
          totalVotes: Math.max(
            Number(existingElection?.totalVotes ?? 0),
            recordedTotalVotes,
          ),
        },
      });
  ...
  async startEventListeners() {
    ...
    const registerListener = (
      ...
      const listener: ChainListener = (...args) => {
        handler(...args).catch((error: unknown) => {
          this.logger.error(
            this.describeSyncError(`${eventName} listener failed`, error),
          );
        });
      };

      contract.on(eventName, listener);
      this.chainListeners.push({ eventName, listener });
    };

    registerListener("ElectionCreated", async (electionId) => {
      await this.syncElection(Number(electionId));
    });

    registerListener("ElectionClosed", async (electionId) => {
      await this.syncElection(Number(electionId));
    });

    registerListener("VoterAuthorized", async (electionId) => {
      await this.syncElectionEvents(Number(electionId));
    });

    registerListener("VoterRevoked", async (electionId) => {
      await this.syncElectionEvents(Number(electionId));
    });
    ...
    registerListener(
      ...
      async (electionId, voter, event) => {
        ...
        await this.refreshElectionVoteCounts(dbElection.id);
      },
    );
