...
// Nếu muốn hiện chiếc khóa cho TOÀN BỘ API trong controller này, hãy đặt ở đây:
...
export class ElectionsController {
  ...
  @ApiSecurity("admin-token") // THÊM DÒNG NÀY: Để hiện biểu tượng khóa
  ...
  @ApiOkResponse({ type: AdminActionLogResponseDto })
  @ApiSecurity("admin-token") // THÊM DÒNG NÀY
  @UseGuards(AdminTokenGuard)
  ...
  @Post(":id/sync")
  @ApiOperation({
    summary: "Sync mot election cu the tu blockchain vao database",
  })
  @ApiParam({ name: "id", example: 0, description: "Contract election id" })
  @ApiOkResponse({ type: SyncResponseDto })
  @ApiSecurity("admin-token") // THÊM DÒNG NÀY
