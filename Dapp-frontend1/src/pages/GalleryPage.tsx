...
function ProposalCard({
  ...
  const { t } = useI18n();
  const statusLabel = formatStatusLabel(proposal.displayStatus, t);
  const hasVotes = proposal.totalVotes > 0;
  const leading = hasVotes
    ? formatCandidateName(proposal.leadingOption, t)
    : t("common.none");
  const footer =
    hasVotes && proposal.leadingOption
      ? `${statusLabel} - ${proposal.leadingPercentage}% ${leading}`
      : hasVotes
        ? `${statusLabel} - ${t("gallery.votesRecorded", { count: proposal.totalVotes })}`
      : `${statusLabel} - ${t("gallery.noVotes")}`;
    ...
    <div className="mx-auto max-w-7xl px-6 py-10 lg:px-10 lg:py-16">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <select
            className="field-input filter-select min-w-[180px]"
            value={status}
