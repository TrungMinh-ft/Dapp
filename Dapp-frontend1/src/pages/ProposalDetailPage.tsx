...
function getNormalizedEndTime(endTime: string) {
  const rawEndTime = Number(endTime);
  return rawEndTime < 10000000000 ? rawEndTime * 1000 : rawEndTime;
}
...
export function ProposalDetailPage() {
  ...
  const isExpired = useMemo(() => {
    ...
    return getNormalizedEndTime(proposal.endTime) <= Date.now();
  }, [proposal, countdown]);

  const showResults = Boolean(proposal?.isClosed || isExpired);

  const resultRows = useMemo(() => {
    if (!proposal) return [];
    const countedTotal = proposal.candidates.reduce((sum, candidate) => sum + Number(candidate.voteCount || 0), 0);
    const totalVotes = Math.max(Number(proposal.totalVotes || 0), countedTotal);
    ...
    return [...proposal.candidates]
      ...
      .map((candidate, index) => ({
        ...candidate,
        rank: index + 1,
        percentage: totalVotes === 0 ? 0 : Number(((candidate.voteCount * 100) / totalVotes).toFixed(2)),
        totalVotes,
      }));
  }, [proposal]);
  ...
  async function submitVote() {
    ...
    try {
      ...
      const nextDetail = await api.getElectionDetail(proposal.contractElectionId);
      setProposal(nextDetail);
      ...
      <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <aside className="panel p-10 shadow-2xl">
          <h2 className="mb-8 flex items-center gap-4 font-heading text-2xl uppercase">
            <ShieldCheck className="text-cyan-400" /> {showResults ? t("vote.resultsTitle") : t("vote.panelTitle")}
          </h2>
          ...
          {showResults ? (
            <div className="space-y-5">
              <div className="flex justify-between rounded-xl border border-accent/20 bg-accent/10 p-4">
                <span className="text-xs uppercase opacity-60">{t("vote.totalVotes")}</span>
                <span className="font-mono text-sm font-bold text-accent">{resultRows[0]?.totalVotes ?? 0}</span>
              </div>
              {resultRows.map((candidate) => (
                <div key={candidate.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                  <div className="mb-3 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-full border border-accent/30 font-mono text-xs text-accent">
                        {candidate.rank}
                      </span>
                      <span className="font-heading text-lg uppercase">{formatCandidateName(candidate.name, t)}</span>
                    </div>
                    <span className="font-mono text-sm text-white">
                      {candidate.voteCount} / {candidate.percentage}%
                    </span>
                  </div>
              ...
              <p className="border-l-2 border-accent pl-3 text-xs italic text-copy">
                {proposal?.isClosed ? t("vote.resultsClosed") : t("vote.resultsEnded")}
              </p>
            </div>
          ) : (
            <>
